...
extension CAIMMetalDrawable
{
    ...
    // MARK: - vertex sampler
    public func setVertexSamplerState( _ sampler: MTLSamplerState?, index: Int ) {
        encoder?.setVertexSamplerState( sampler, index: index )
    }
    public func setVertexSamplerState( _ sampler: MTLSamplerState?, lodMinClamp:Float, lodMaxClamp:Float, index:Int ) {
    ...
    public func setVertexSamplerStates( _ samplers: [MTLSamplerState?], range:Range<Int> ) {
        encoder?.setVertexSamplerStates( samplers, range: range )
    }
    public func setVertexSamplerStates( _ samplers:[MTLSamplerState?], lodMinClamps:[Float], lodMaxClamps:[Float], range:Range<Int> ) {
        encoder?.setVertexSamplerStates( samplers, lodMinClamps: lodMinClamps, lodMaxClamps: lodMaxClamps, range: range )
    }
    // MARK: - vertex texture
    public func setVertexTexture( _ texture:MTLTexture?, index idx:Int ) {
        encoder?.setVertexTexture( texture, index:idx )
    }
    public func setVertexTextures( _ textures: [MTLTexture?], range:Range<Int> ) {
        encoder?.setVertexTextures( textures, range: range )
    }
    
    // MARK: - fragment buffer
    public func setFragmentBuffer( _ buffer:MTLBuffer, offset:Int = 0, at idx:Int ) {
        encoder?.setFragmentBuffer( buffer, offset:offset, index: idx )
    }
    public func setFragmentBuffer( _ buffer_base:CAIMMetalBufferBase, offset:Int = 0, at idx:Int ) {
        encoder?.setFragmentBuffer( buffer_base.metalBuffer, offset:offset, index: idx )
    }
    public func setFragmentBuffer( _ mem:CAIMMetalBufferAllocatable, offset:Int = 0, at idx:Int ) {
        encoder?.setFragmentBuffer( mem.metalBuffer, offset:offset, index: idx )
    }
    public func setFragmentBuffers( _ buffers:[MTLBuffer?], offsets: [Int], range: Range<Int> ) {
        encoder?.setFragmentBuffers( buffers, offsets: offsets, range:range )
    }
    ...
    public func setFragmentSamplerState( _ sampler: MTLSamplerState?, lodMinClamp:Float, lodMaxClamp:Float, index:Int ) {
    ...
    public func setFragmentSamplerStates( _ samplers: [MTLSamplerState?], range:Range<Int> ) {
        encoder?.setFragmentSamplerStates( samplers, range: range )
    }
    public func setFragmentSamplerStates( _ samplers:[MTLSamplerState?], lodMinClamps:[Float], lodMaxClamps:[Float], range:Range<Int> ) {
        encoder?.setFragmentSamplerStates( samplers, lodMinClamps: lodMinClamps, lodMaxClamps: lodMaxClamps, range: range )
    }
    // MARK: - fragment texture
    public func setFragmentTexture( _ texture:MTLTexture?, index idx:Int ) {
        encoder?.setFragmentTexture( texture, index:idx )
    }
    public func setFragmentTextures( _ textures: [MTLTexture?], range:Range<Int> ) {
        encoder?.setFragmentTextures( textures, range: range )
    }
}
...
public class CAIMMetalShape<T> : CAIMMemory4K, CAIMMetalDrawable
{
    ...
    public func draw() {
        setVertexBuffer( self.metalBuffer, at: _buffer_index )
    }
}
