...
class ImageToolBox
{
    ...
    static func fillDomeFast(_ img:CAIMImage, cx:Int, cy:Int, radius:Int, color:CAIMColor, opacity:Float) {
        ...
        // ■効率化1: 同じ計算はループ前にしておき、変数に入れておく
        let r2:Int = radius * radius
        let k:Float = 1.0 / Float(r2) * Float(M_PI) / 2.0
        
        // min_x~max_x, min_y~max_yまでの範囲を塗る
        for y in min_y ... max_y {
            // ■効率化2: y-cyの2乗計算はxの1ラインでは全て同じなので予め計算してdy2に格納
            // ■効率化3: CPUではFloatよりIntのほうが計算が高速。dy2の型はIntにする
            let dy2:Int = (y-cy)*(y-cy)
            for x in min_x ... max_x {
                // ■効率化4: こちらもdx2の型はIntにする
                let dx2:Int = (x-cx)*(x-cx)
                
                // ■効率化5: sqrt(平方根）関数は遅い。中心点からの距離は2乗のまま用いる.
                // 以下で距離を比較するが、2乗のまま比較をしても、sqrtを使っても数学的に同じ結果となる
                let dist2:Int = dx2 + dy2
                
                // 中心点(cx, cy)からの距離が半径radius以内なら塗る(2乗のまま)
                if( dist2 <= r2 ) {
                    // 中心からの距離でcosを計算してαとして用いる(kは効率化1で事前計算しているものを使う)
                    let alpha:Float = cos(Float(dist2) * k) * opacity
                    // ■効率化6: 下記の色計算で毎度1.0-alphaをするのはもったいないので先に計算してbetaとしておく
                    let beta:Float = 1.0-alpha
                    
                    // ■効率化7: mat[y][x]はC言語のメモリポインタで直接メモリに触れる。アクセス回数が多いと遅くなる。
                    // そこでcに一度色を入れ、計算中でmat[y][x]を使う回数を減らす
                    let c:CAIMColor = mat[y][x]
                    
                    // ■効率化8: こちらもmat[y][x]へのアクセスを減らすため、
                    // 計算したRGBAの値でCAIMColorを作り、mat[y][x]に1度に代入する
                    mat[y][x] = CAIMColor(R: color.R * alpha + c.R * beta,
                                          G: color.G * alpha + c.G * beta,
                                          B: color.B * alpha + c.B * beta,
                                          A: color.A * alpha + c.A * beta)
                }
            }
        }
    }
